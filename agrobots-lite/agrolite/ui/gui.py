...
from agrolite.models.field import CellKind
from agrolite.models.robot import RobotStatus, build_robot
from agrolite.models.task import Task, TaskKind, TaskStatus
...
class _Modal(tk.Toplevel):
    ...
    def add_buttons(self) -> None:
        ...
        ttk.Button(bar, text="ОК", command=self._submit).pack(side="right", padx=(0, 8))
...
class _RobotDialog(_Modal):
    def __init__(self, parent, sim, default: Optional[Tuple[int, int]]):
        ...
        self.x.set(default[0] if default else 0)
        self.y.set(default[1] if default else 0)
...
class _RegionTaskDialog(_Modal):
    def __init__(self, parent, sim, count: int):
        super().__init__(parent, "Наряды на область")
        self.sim = sim
        self.kind = ttk.Combobox(
            self.body, values=[label for _k, label in TASK_KINDS], state="readonly"
        )
        ...
        self.duration.set(2)
        _add_row(self.body, 0, "Операция", self.kind)
        _add_row(self.body, 1, "Приоритет (1 — срочно)", self.priority)
        _add_row(self.body, 2, "Срок (день)", self.deadline)
        _add_row(self.body, 3, "Длительность (тиков)", self.duration)
        tk.Label(
            self.body, text=f"Клеток в области: {count}", bg=PANEL_BG, fg=MUTED, anchor="w"
    ...
    def _submit(self) -> None:
        try:
            ...
            return
        kind = TASK_KINDS[self.kind.current()][0]
        self.result = (kind, priority, deadline, duration)
        self.destroy()
...
class AgroBotsGUI:
    def __init__(self) -> None:
        self.sim = build_default_sim()
        self.auto = False
        self.selected: Optional[Tuple[int, int]] = None
        self._drag_origin: Optional[Tuple[int, int]] = None

        self.root = tk.Tk()
        self.root.title("AgroBots Lite — роботизированное агрохозяйство")
    ...
    def _init_style(self) -> None:
        ...
        style.configure(
            "Treeview", background="#262838", foreground=TEXT,
            fieldbackground="#262838", rowheight=24, borderwidth=0,
        )
        style.map("Treeview", background=[("selected", "#3b5bdb")])
        style.configure("Treeview.Heading", background="#1e1f2b", foreground=ACCENT, relief="flat")
        style.configure("TNotebook", background=WIN_BG, borderwidth=0)
        style.configure("TNotebook.Tab", background="#1e1f2b", foreground=MUTED, padding=(14, 6))
        style.map(
            "TNotebook.Tab",
    ...
    def _build_body(self) -> None:
        ...
        self.robots_tree = self._make_tree(
            notebook,
            ("id", "type", "name", "pos", "bat", "status", "task", "done"),
            ("ID", "Тип", "Имя", "Поз", "Батарея", "Статус", "Наряд", "Готово"),
            (36, 90, 120, 64, 80, 96, 64, 64),
        )
    ...
    def _build_legend(self, parent) -> None:
        ...
        tk.Label(legend, text="клик — выбор клетки · выделение рамкой — наряды на область",
    ...
    def _clamp_cell(self, px: int, py: int) -> Tuple[int, int]:
        x = (px - 30) // CELL
        y = (py - 20) // CELL
        x = max(0, min(self.sim.field.width - 1, x))
        y = max(0, min(self.sim.field.height - 1, y))
        return (x, y)

    def _on_press(self, event) -> None:
        self._drag_origin = self._cell_at(event.x, event.y)

    def _on_drag(self, event) -> None:
        if self._drag_origin is None:
            return
        self._draw_drag_rect(self._drag_origin, self._clamp_cell(event.x, event.y))

    def _on_release(self, event) -> None:
        origin = self._drag_origin
        self._drag_origin = None
        self.canvas.delete("dragsel")
        if origin is None:
            return
        current = self._clamp_cell(event.x, event.y)
        if origin == current:
            self.selected = origin
            self.refresh()
            return
        cells = self._cells_in_region(origin, current)
        if not cells:
            messagebox.showinfo(
                "Область пуста", "В выбранной области нет проходимых клеток.", parent=self.root
            )
            self.refresh()
            return
        self._create_region_tasks(cells)

    def _draw_drag_rect(self, c0: Tuple[int, int], c1: Tuple[int, int]) -> None:
        self.canvas.delete("dragsel")
        x0 = 30 + min(c0[0], c1[0]) * CELL
        y0 = 20 + min(c0[1], c1[1]) * CELL
        x1 = 30 + (max(c0[0], c1[0]) + 1) * CELL
        y1 = 20 + (max(c0[1], c1[1]) + 1) * CELL
        self.canvas.create_rectangle(
            x0 + 1, y0 + 1, x1 - 1, y1 - 1, outline=ACCENT, width=2, dash=(4, 3), tags="dragsel"
        )

    def _cells_in_region(self, c0: Tuple[int, int], c1: Tuple[int, int]) -> list:
        xs = range(min(c0[0], c1[0]), max(c0[0], c1[0]) + 1)
        ys = range(min(c0[1], c1[1]), max(c0[1], c1[1]) + 1)
        return [(x, y) for y in ys for x in xs if self.sim.field.passable(x, y)]

    def _create_region_tasks(self, cells: list) -> None:
        config = _RegionTaskDialog(self.root, self.sim, len(cells)).show()
        if config is None:
            return
        kind, priority, deadline, duration = config
        for x, y in cells:
            self.sim.add_task(
                Task(kind, x, y, priority=priority, deadline=deadline, duration=duration)
            )
        self.refresh()
    ...
    def _draw_field(self) -> None:
        ...
        for y in range(field.height):
            ...
            for x in range(field.width):
                ...
                c.create_rectangle(x0, y0, x0 + CELL, y0 + CELL, fill=fill, outline=CELL_LINE)
    ...
    def _draw_task(self, task) -> None:
        ...
        self.canvas.create_rectangle(x0 + 6, y0 + 6, x0 + CELL - 6, y0 + CELL - 6,
